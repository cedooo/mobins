 ...
 *
 */
public class IntValueRangeMatch extends ResultMatch{
	static final private Logger LOG = LoggerFactory.getLogger(IntValueRangeMatch.class.getClass());
	@Override
	protected MatchResult doIt() {
		...
		if(anaInfo.getKeyRegex()!=null){
			...
			try{
				range[0] = (int)Double.parseDouble(anaInfo.getValCompareMin());
				range[1] = (int)Double.parseDouble(anaInfo.getValCompareMax());
			}catch (Exception e){
			...
			List<Integer> inrangeList = new ArrayList<Integer>();
			List<Integer> outrangeList = new ArrayList<Integer>();
			...
			for (String str : dataArray) {
				...
				if(matcher.matches()){
					...
					for (int i = 1; i <= count; i++) {
						...
						try{
							val = (int)Double.parseDouble(matched);
